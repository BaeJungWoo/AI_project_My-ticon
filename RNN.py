...
class RNN(nn.Module):
  ...
  def train_model(self, x_train, y_train, num_epochs, batch_size, learning_rate):
    ...
    for e in range(num_epochs):
      ...
      for batch_idx in range(iter_per_epoch-1):
        self.optimizer.zero_grad()
        out = x_train[batch_idx:batch_idx+batch_size,:]
        out = torch.Tensor(out).long().to(self.device)
        out = self.forward(out)
        batch_labels = torch.Tensor(y_train[batch_idx:batch_idx+batch_size,:].reshape(-1)).long().to(self.device)
  ...
  def predict(self, X, y, batch_size, return_preds=False):
    ...
    with torch.no_grad():
      ...
      for batch_idx in range(iter_per_epoch-1):
        out = X[batch_idx:batch_idx+batch_size,:]
        out = torch.Tensor(out).long().to(self.device)
        out = self.forward(out)
        batch_labels = torch.Tensor(y[batch_idx:batch_idx+batch_size,:].reshape(-1)).long().to(self.device)
        loss = self.loss_function(out, batch_labels)
        total_loss += loss
        max_vals, max_indices = torch.max(out,1)
        preds[batch_idx:batch_idx+batch_size] = max_indices.float()
